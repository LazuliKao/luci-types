...
import { fetchDocPage, fetchSubPageItems, extractDocSection } from "./lib/doc.js";
import {
  parseLTypeMembers,
...
async function main(): Promise<void> {
  ...
  printExtra("Present in LuCI namespace but NOT documented as type defs", extraTypes);
  ...
  const missingModules = [...docModuleSet]
    ...
    .sort();
  const extraModules = declaredModules.filter((n) => !docModuleSet.has(n)).sort();
  ...
  for (const mod of declaredModules) {
    ...
    const missingSub = [...subDocSet].filter((n) => !subDeclaredSet.has(n)).sort();
    ...
    printExtra(
      `Declared in LuCI.${mod} but NOT documented`,
      extraSub,
    );
    ...
    for (const child of uniqueChildren) {
      ...
      const childTypeDefs = childPage.items
        .filter((d) => d.section === "type-def")
        .map((d) => d.name);
      const childDocSet = new Set([...childMembers, ...childMethods, ...childTypeDefs]);

      const childDeclared = parseChildMembers(mod, child);
      const childDeclaredSet = new Set(childDeclared);

      const missingChild = [...childDocSet].filter((n) => !childDeclaredSet.has(n)).sort();
      const extraChild = childDeclared.filter((n) => !childDocSet.has(n)).sort();
