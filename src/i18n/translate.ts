...
export async function translateTranslations(
	options: TranslateTranslationsOptions,
): Promise<TranslateTranslationsResult> {
	...
	for (let index = 0; index < pending.length; index += batchSize) {
		...
		for (const source of batch) {
			...
			if (target === undefined) {
				throw new Error(
					`Translator did not return a translation for: ${source}`,
				);
			}

			if (target.trim() === "") {
				throw new Error(
					`Translator returned an empty translation for: ${source}`,
				);
			}
