...
export class OpenAICompatibleTranslator implements Translator {
  ...
  constructor(options: OpenAICompatibleTranslatorOptions = {}) {
    ...
    this.#apiKey = apiKey;
    this.#apiUrl = normalizeChatCompletionsUrl(options.apiUrl ?? process.env.OPENAI_API_URL ?? defaultApiUrl);
  ...
  async translate(texts: readonly string[]): Promise<Map<string, string>> {
    ...
    return new Map(texts.map((text, index) => {
      const translation = translations[index];

      if (translation === undefined || translation.trim() === "") {
        throw new Error(`Translator returned an empty translation for: ${text}`);
      }
  ...
  async #createCompletion(texts: readonly string[]): Promise<string> {
    const response = await fetch(this.#apiUrl, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: "user",
            content: texts.map((text, index) => `${index + 1}. ${JSON.stringify(text)}`).join("\n"),
          },
        ],
      }),
    });
    ...
    if (!response.ok) {
      throw new Error(`OpenAI-compatible API request failed: ${response.status} ${response.statusText} ${await response.text()}`);
    }
...
function parseTranslationArray(content: string, expectedLength: number): string[] {
  ...
  if (parsed.length !== expectedLength) {
    throw new Error(`Translator returned ${parsed.length} item(s), expected ${expectedLength}.`);
  }
